...
This tool scans for Monte Carlo result archives and ST-LRPS run directories and labels
each one ``trusted`` / ``quarantined`` / ``rerun_required`` with explicit, item-tagged
reasons:
...
Usage
-----
    python -m tools.audit_legacy_results [--root outputs] [--output outputs/audit/legacy_results_manifest.json]

The classification logic (``classify_mc_archive`` / ``classify_st_lrps_run``) is pure and
unit-tested; ``main`` only handles discovery and I/O.
"""
...
def audit_root(root: Path) -> dict[str, Any]:
    ...
    buckets = {TRUSTED: [], QUARANTINED: [], RERUN: []}
    for e in entries:
