...
| 7 | Domain guard violation (state outside the surrogate's trained altitude/radius envelope) | RuntimeError (`strict_domain=true`) | Warn; sample flagged `domain_exit=true`, result marked low-confidence | Enforced in benchmark (`allow_domain_extrapolation=false`); enforced in frozen search by stage-1 guard scoring plus the R27 candidate-promotion choke point |
