...
- `ui/app.py` + `ui/pages/`, `ui/core/`, `ui/theme/` — PySide6 desktop UI
  (mission simulator). The calm *Lunar Graphite* theme is centralized in
  `ui/core/ui_commons.py` (the `THEME` widget palette and the `ORBIT_THEME`
  OpenGL-preview palette), while the global stylesheet is generated by
  the binding-neutral `ui_foundation` package. Compatibility exports remain in
  `ui/theme/`. See
  [docs/UI_THEME.md](UI_THEME.md).
