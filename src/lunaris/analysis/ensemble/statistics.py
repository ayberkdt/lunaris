...
Post-processes a batch-propagated ensemble into actionable statistics. Random
Monte Carlo is one supported sampling design; the same statistics also apply to
Latin Hypercube, Sobol, and other ensemble designs stored in ``MCRunResult``.
...
class ImpactStatistics:
    ...
    Attributes
    ----------
    n_total       : total number of ensemble samples
...
def ric_basis_from_state(mean_state: F64Array) -> F64Array:
    ...
    bad = (
        (r_norm <= _RIC_NORM_ATOL)
        | (v_norm <= _RIC_NORM_ATOL)
        | (sin_angle <= _RIC_SIN_ANGLE_ATOL)
    )
    if np.any(bad):
        idx = int(np.nonzero(bad)[0][0])
        raise ValueError(
            "RIC frame is undefined for degenerate mean state at epoch "
            f"{idx}: |r|={r_norm[idx]:.6e} m, |v|={v_norm[idx]:.6e} m/s, "
            f"|r x v|={h_norm[idx]:.6e} m^2/s."
        )
