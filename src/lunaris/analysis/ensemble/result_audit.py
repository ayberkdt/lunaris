"""Audit pre-contract batch-ensemble / ST-LRPS results into a trust manifest.
...
This module scans for batch-propagation archives and ST-LRPS run directories and
...
Verdict ladder (most-to-least severe):
  ...
  * ``invalid``       — structurally not a usable result: a corrupt schema version,
                        ...
                        manifest fields (not produced by a complete batch engine
...
# Hash-provenance keys written by the historical MonteCarloEngine.run path (§7).
# Presence of any one
...
# declares the v2 contract but was not produced by a complete batch engine
...
def classify_mc_archive(metadata: dict[str, Any], *, has_impacts: bool) -> tuple[str, list[str]]:
    """Classify one batch-propagation archive from its manifest metadata.
    ...
    if missing:
        ...
        return INVALID, [
            ...
            + ", ".join(sorted(missing))
            + " (not a complete batch-propagation run)"
        ]
