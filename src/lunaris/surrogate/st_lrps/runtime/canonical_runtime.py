#!/usr/bin/env python3
"""
canonical_runtime.py - THE canonical ST-LRPS inference runtime (roadmap R11).

Single source of truth for every inference-path responsibility: artifact
loading, model-kind validation (potential_autograd only; archived kinds are
rejected fail-closed), scaler loading, the domain guard (numpy and torch
paths), scalar-potential prediction, autograd acceleration, residual+baseline
composition, and paper-safe metadata/provenance. No other module may
re-implement scaler / domain-guard / model-kind logic; adapters
(``lunaris.surrogate.runtime.gravity_provider``) delegate here, and the old
``force_model`` module is a dynamic compat shim over this one.

The ONLY sanctioned exception is the explicitly-guarded legacy path in
``gravity_provider.from_model_dir`` for pre-contract artifacts (no declared
artifact_contract / target_contract / runtime_model_kind): it warns loudly,
refuses contract-era artifacts, and exists so historical experimental runs
remain loadable in research mode.
...
def enforce_altitude_envelope_torch(
    ...
    *,
    ...
    """Canonical torch-path training-envelope guard (R11 single source).

    Mirrors :meth:`SurrogateForceModel._enforce_domain` semantics for tensor
    entry points: hard-fail when ``strict``, otherwise warn once. No-op when
    the envelope is unknown (never blocks on unknown bounds). Returns the new
    warn-once state so stateless callers can persist it.
    """
    if alt_min_km is None or alt_max_km is None:
    ...
    if lo >= float(alt_min_km) - tol_km and hi <= float(alt_max_km) + tol_km:
        return already_warned
    msg = (
        f"{caller}: input altitude range [{lo:.1f}, {hi:.1f}] km is outside the surrogate "
        f"training envelope [{float(alt_min_km):.1f}, {float(alt_max_km):.1f}] km; GPU tensor "
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="Smoke-test st_lrps.runtime.canonical_runtime")
