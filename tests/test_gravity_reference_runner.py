...
def test_trajectory_validation_end_to_end(tmp_path: Path) -> None:
    ...
    The reference was generated by an external library (pyshtools) and a
    separately configured SciPy DOP853 run; it is not an independent-integrator
    or physical rotating-frame validation. The runner re-propagates with the
