...
from lunaris.batch import storage as batch_storage
...
def test_auto_hdf5_uses_disk_only_above_memory_limit() -> None:
    small = _batch_config(output_format="hdf5", memory_gb=1.0)
    mode, result_bytes, limit_bytes = _resolve_result_storage(small, n_steps=10)
    assert mode == "memory"
    assert result_bytes < limit_bytes

    large = _batch_config(output_format="hdf5", memory_gb=1.0e-9)
...
def test_large_auto_npz_is_rejected_with_hdf5_guidance() -> None:
    cfg = _batch_config(output_format="npz", memory_gb=1.0e-9)
...
def test_low_available_ram_forces_disk_under_config_cap(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        batch_storage, "_available_host_memory_bytes", lambda: 1024
    )
...
def test_explicit_memory_warns_when_exceeding_safety_budget(monkeypatch) -> None:
    ...
    cfg = _batch_config(output_format="hdf5", storage_mode="memory", memory_gb=1.0)
...
def test_missing_psutil_disables_safety_factor(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        batch_storage, "_available_host_memory_bytes", lambda: None
    )
    cfg = _batch_config(output_format="hdf5", memory_gb=1.0)
...
def test_disk_mode_never_allocates_full_trajectory(monkeypatch) -> None:
    ...
    monkeypatch.setattr(batch_storage.np, "empty", fail_allocation)
