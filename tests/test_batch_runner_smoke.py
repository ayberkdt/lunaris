...
def test_batch_runner_smoke_summary_only_budget(tmp_path: Path) -> None:
    # R23: verify the summary-only mode writes the reduced output schema
    # without blowing up the memory budget.
    output_path = tmp_path / "batch_smoke_summary.npz"
    ...
    assert "Y_summary" in data
    assert data["Y_summary"].ndim == 3
    assert data["Y_summary"].shape[1] == 2
    assert data["impact_flags"].shape == (2,)
