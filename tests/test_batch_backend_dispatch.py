"""
Live batch backend dispatch for torch_cuda_sh (task §15 / §10 / §16E).

Two layers, both CPU-only (CUDA availability is monkeypatched):

1. ``resolve_batch_backend_policy`` integration — the *real* policy resolver (not
...
def test_batch_body_vector_policy_includes_relativity_external_terms() -> None:
    from lunaris.batch.requirements import _need_body_vectors
...
def _torch_plan_engine(monkeypatch, tmp_path, *, batch_backend="torch_cuda_sh", degree=100):
    ...
    class DummyTorchSH:
        def __init__(self, dyn, batch_cfg, flags, **k) -> None:
    ...
    monkeypatch.setattr(batch_prop, "GPUBatchPropagator", DummyNumbaGPU)
...
def test_engine_dispatches_torch_sh_and_not_numba_or_cpu(monkeypatch, tmp_path: Path) -> None:
    ...
    assert engine._backend_plan.final_backend == BatchBackend.GPU_TORCH_SH
...
def test_engine_explicit_numba_high_degree_dispatches_torch(monkeypatch, tmp_path: Path) -> None:
    ...
    engine, constructed, DummyTorchSH = _torch_plan_engine(
        monkeypatch, tmp_path, batch_backend="numba_cuda_sh", degree=100
    )
    from lunaris.batch.engine import BatchPropagationEngine
...
def test_engine_dispatches_torch_cpu_sh_with_cpu_device(monkeypatch, tmp_path: Path) -> None:
    ...
    assert engine._backend_plan.final_backend == BatchBackend.TORCH_CPU_SH
...
def _patch_dynamics_gravity_capture(monkeypatch) -> dict:
    ...
    # lunaris.batch.engine compatibility shim.
...
def test_backend_display_name_distinguishes_torch_cpu_from_cuda() -> None:
    ...
    from lunaris.batch.engine import _BACKEND_DISPLAY_NAMES
