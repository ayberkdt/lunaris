...
produced). ``EnsembleReport.impacts`` is ``None`` in the latter two cases so no
...
def _result(*, impacts: bool, diagnostics: dict | None = None) -> BatchPropagationResult:
...
def test_explicit_compute_impacts_false_overrides_manifest() -> None:
    result = _result(impacts=True, diagnostics={"compute_impact_statistics": True})
    stats = compute_ensemble_report(result, compute_oe=False, compute_impacts=False)
    assert stats.impact_status == IMPACT_DISABLED
    assert stats.impacts is None
