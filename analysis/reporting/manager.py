...
from loaders.io_helpers import find_lunar_map_path
...
def figure_run_config_page(
    ...
    *,
    ...
    """Build the 'Configuration & Performance' page (integrator, spacecraft, timing, events)."""
    apply_rcparams(DEFAULT_STYLE)
    ...
    # Prefer strict output dt key, keep legacy fallback for now
    ...
    # robust counting for array-like
    ...
    # --- Layout ---
    fig = plt.figure(figsize=(8.27, 11.69))
    fig.suptitle("CONFIGURATION & PERFORMANCE", fontsize=16, fontweight="bold", y=0.95, color="#334155")

    config_items = [
        ("Integrator", integrator),
        ("Tolerance (rtol/atol)", f"{rtol} / {atol}"),
        ("Gravity Model", f"Spherical Harmonics (Degree {sh_degree})"),
        ("Central Body GM", f"{mu:.4e} m³/s²"),
        ("Output Step", "Auto" if out_dt_s is None else f"{out_dt_s} s"),
    ]
    ax_cfg = fig.add_axes([0.1, 0.65, 0.4, 0.2])
    draw_kv_block(ax_cfg, "Simulation Settings", config_items)

    sc_items = [
        ("Dry Mass", f"{mass} kg"),
        ("Cross-Section Area", f"{area} m²"),
        ("Drag Coeff (Cd)", f"{cd}"),
        ("Reflectivity (Cr)", f"{cr}"),
    ]
    ax_sc = fig.add_axes([0.55, 0.65, 0.35, 0.2])
    draw_kv_block(ax_sc, "Spacecraft Parameters", sc_items)

    fig.add_artist(
        plt.Line2D([0.1, 0.9], [0.60, 0.60], transform=fig.transFigure, color="#E2E8F0", linestyle="--")
    )

    # --- Event statistics ---
    ax_ev = fig.add_axes([0.1, 0.40, 0.8, 0.15])
    ax_ev.axis("off")
    ax_ev.text(0.0, 1.0, "Mission Event Log", transform=ax_ev.transAxes, fontsize=12, fontweight="bold", color="#334155")

    def _draw_counter(x: float, label: str, val: Any, color: str) -> None:
        ax_ev.text(x, 0.6, label, ha="center", fontsize=9, color="#64748B")
        ax_ev.text(x, 0.3, str(val), ha="center", fontsize=18, fontweight="bold", color=color)

    _draw_counter(0.2, "PERISELENES", n_peri, "#059669")
    _draw_counter(0.5, "APOSELENES", n_apo, "#D97706")

    imp_color = "#DC2626" if has_impact else "#94A3B8"
    imp_text = "YES" if has_impact else "NO"
    _draw_counter(0.8, "IMPACT", imp_text, imp_color)

    # --- Computational performance ---
    perf_items = [
        ("Propagation Time", f"{t_prop:.4f} s"),
        ("Total Wall Time", f"{t_wall:.4f} s"),
        ("Time Steps Computed", f"{n_steps:,}"),
    ]
    ax_perf = fig.add_axes([0.1, 0.15, 0.8, 0.2])
    draw_kv_block(ax_perf, "Computational Performance", perf_items)

    if pdf_or_out_dir_hint:
        fig.text(0.5, 0.03, f"Source: {Path(pdf_or_out_dir_hint).name}", ha="center", fontsize=8, color="#CBD5E1")
